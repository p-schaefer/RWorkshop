...
scatter.smooth(x=trenddata$Intolerants.Richness, y=fitted(watershed.model2))
xyplot(trenddata$Intolerants.Richness~trenddata$MonYr|trenddata$Zone,groups=trenddata$Site)
...
top.models2<-get.models(mm2,subset=mm2$delta<6)
...
mm1.av<-model.avg(get.models(mm1,subset=cumsum(mm2$weight)<=0.95), revised.var=T)
mm2.av<-model.avg(get.models(mm2,subset=cumsum(mm2$weight)<=0.95), revised.var=T)
...
#The average number of intolerant taxa iis 1.6 in the L zone, and ~6.2 for M and U. Significant differences detected between zones.
#
...
#In the L zone, intolerants richness is decreasing by ~0.1% every year or multiplied by ~0.99. Alternatively, in the upper zone,
