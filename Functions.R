ci_fun2<-function(model,model.variable,data=NULL,data.variable="Site",plot=T,stat="median",level=0.95,oddsRatio=F,nsim=1000,sig.only=FALSE,...){
  require(plotrix)
  require(merTools)
  ...
  if (plot==T) {
    if (sig.only==T){
      ...
      if (!is.null(data) & !is.null(data$Zone)){
        colours<-data$Zone[sapply(as.character(plot.output$groupID),function(x) match(x,as.character(eval(parse(text=paste0('data$',data.variable))))))]
      }
      ...
      plotCI(x=1:sitenum,y=plot.output[, stat],
             ...
             col=if (!is.null(data)& !is.null(data$Zone)) {col=colours} else {"black"})
